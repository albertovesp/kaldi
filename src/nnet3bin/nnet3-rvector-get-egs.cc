  ...
  if (!range_rxfilename.empty()) {
    ...
    while (std::getline(range_input.Stream(), line)) {
      ...
      if (!ConvertStringToInteger(fields[1], &(chunk_info->output_archive_id))
        ...
        || !SplitStringToFloats(params_str, ",", false, &(chunk_info->params)))
        KALDI_ERR << "Expected integer for output archive and float for params in range file.";
...
static void WriteExamples(const MatrixBase<BaseFloat> &feats,
  ...
  for (std::vector<ChunkInfo *>::const_iterator it = chunks.begin();
    ...
    if (num_rows < chunk->num_frames) {
      ...
      for(int32 i = 0; i < chunk->params.size(); i++) {
        Matrix<BaseFloat> param(1,1);
        param(0,0) = chunk->params[i];
        std::string idx = std::to_string(i+1);
        eg.io.push_back(NnetIo("output"+idx, 0, param));
      }
      if (compress)
        eg.Compress();
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Get examples for training an nnet3 neural network for the rvector\n"
        "system.  Each output example contains a chunk of features from some\n"
        "utterance along with RIR label 7 room config parameters.  The location\n"
        "and length of the feature chunks are specified in the 'ranges' file.\n"  
        "Each line is interpreted as follows:\n"
        "  <source-utterance> <relative-output-archive-index> "
        "<absolute-archive-index> <start-frame-index> <num-frames> <spk-lbl>"
        "<params>\n"
        ...
        "For example:\n"
        "  utt1  3  13  65  300  0 <params>\n"
        "  utt1  0  10  50  400  1 <params>\n"
        "  utt2  ...\n"
        "\n"
        "Usage:  nnet3-rvector-get-egs [options] <ranges-filename> "
        "<features-rspecifier> <egs-0-out> <egs-1-out> ... <egs-N-1-out>\n"
        "\n"
        "For example:\n"
        "nnet3-rvector-get-egs ranges.1 \"$feats\" ark:egs_temp.1.ark"
