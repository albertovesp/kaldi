...
#    <utt-id> <local-ark-indx> <global-ark-indx> <start-frame> <end-frame> <params>
# For example:
...
if [ $stage -le 11 ]; then
  ...
  regression_opts="objective-type=quadratic dim=1 input=tdnn7 include-log-softmax=false"
  ...
  output-layer name=output1 $regression_opts
  output-layer name=output2 $regression_opts
  output-layer name=output3 $regression_opts
  output-layer name=output4 $regression_opts
  output-layer name=output5 $regression_opts
  output-layer name=output6 $regression_opts
  output-layer name=output7 $regression_opts
...
if [ $stage -le 12 ]; then
  steps/nnet3/train_raw_dnn.py --stage=$train_stage \
    ...
    --trainer.optimization.minibatch-size=32 \
    --trainer.srand=$srand \
