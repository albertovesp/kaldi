...
if ($num_freetext_states > 1) {
  $transp = 1.0 / ($num_freetext_states-1);
  print "<TopologyEntry>\n";
  print "<ForPhones>\n";
  print "$freetext_phones\n";
  print "</ForPhones>\n";
  print "<State> 0 <PdfClass> <ForwardPdfClass> 0 <SelfLoopPdfClass> 1 ";
  for ($nextstate = 0; $nextstate < $num_freetext_states-1; $nextstate++) { # Transitions to all but last
    # emitting state.
    print "<Transition> $nextstate $transp ";
  }
  print "</State>\n";
  for ($state = 1; $state < $num_freetext_states-1; $state++) { # the central states all have transitions to
    # themselves and to the last emitting state.
    print "<State> $state <ForwardPdfClass> @{[2*$state]} <SelfLoopPdfClass> @{[2*$state+1]} ";
    for ($nextstate = 1; $nextstate < $num_freetext_states; $nextstate++) {
      print "<Transition> $nextstate $transp ";
    }
    print "</State>\n";
  }
  # Final emitting state (non-skippable).
  $state = $num_freetext_states-1;
  print "<State> $state <ForwardPdfClass> @{[2*$state]} <SelfLoopPdfClass> @{[2*$state+1]} <Transition> $state 0.5 <Transition> $num_freetext_states 0.5 </State>\n";
