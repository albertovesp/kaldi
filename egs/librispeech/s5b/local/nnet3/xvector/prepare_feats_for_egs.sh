...
if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $featdir/storage ]; then
  utils/create_split_dir.pl \
    /export/b{14,15,16,17}/$USER/kaldi-data/egs/librispeech/s5b/xvector-$(date +'%m_%d_%H_%M')/xvector_feats/storage $featdir/storage
fi
...
cp $data_in/utt2params $data_out/utt2params
