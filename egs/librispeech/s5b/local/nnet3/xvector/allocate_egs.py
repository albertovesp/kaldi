...
#
#   utt1  0  1  0   65 2 <params>
#   utt1  6  7  160 50 3 <params>
#   utt2  ...
#
# where each line is interpreted as follows:
#  <source-utterance> <relative-archive-index> <absolute-archive-index> \
#    <start-frame-index> <num-frames> <spk-lbl> <params>
# and <params> refer to RIR parameters.
...
def get_args():
    ...
    parser.add_argument("--utt2int-filename", type=str, required=True,
                    ...
                    "<utterance-id> <id>)");
    parser.add_argument("--utt2params-filename", type=str, required=True,
                    help="utt2params file of the features to be used as input (format is: "
                    "<utterance-id> <params>)");
...
def process_args(args):
    ...
    if not os.path.exists(args.utt2params_filename):
        raise Exception("This script expects --utt2params-filename to exist")
...
# Handle utt2params, create params2utt, params
def get_params(utt2params_filename):
    f = open(utt2params_filename, "r")
    if f is None:
        sys.exit("Error opening utt2params file " + str(utt2params_filename))
    utt2params = {}
    for line in f:
        tokens = line.split(' ', 1)
        params = tuple(float(x) for x in tokens[1].split(' '))
        if len(params) != 7:
            sys.exit("bad line in utt2params file " + line)
        utt = tokens[0]
        utt2params[utt] = params
    f.close()
    return utt2params
    # Done utt2params
...
def main():
    ...
    utt2params = get_params(args.utt2params_filename)
    ...
    for job in range(args.num_jobs):
        ...
        for (utterance_index, i, offset) in sorted(this_ranges):
            ...
            params_str = ','.join([str(x) for x in utt2params[utterance_index]])
            print("{0} {1} {2} {3} {4} {5} {6}".format(utterance_index,
                                           i,
                                           archive_index + 1,
                                           offset,
