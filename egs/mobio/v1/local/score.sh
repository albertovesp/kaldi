...
if [ $stage -le 0 ]; then
  ...
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    ...
    if $decode_mbr ; then
      ...
      $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring_kaldi/penalty_$wip/log/best_path_score.LMWT.log \
        lattice-scale --inv-acoustic-scale=LMWT "ark:gunzip -c $dir/lat.*.gz|" ark:- \| \
        lattice-add-penalty --word-ins-penalty=$wip ark:- ark:- \| \
        lattice-1best ark:- ark:- \| \
        nbest-to-linear ark:- ark:/dev/null ark:/dev/null ark,t:$dir/scoring_kaldi/penalty_$wip/lmcost ark,t:$dir/scoring_kaldi/penalty_$wip/accost || exit 1;
      paste -d' ' <(cut -f1,2 -d' ' $dir/scoring_kaldi/penalty_$wip/lmcost) <(cut -f2 -d' ' $dir/scoring_kaldi/penalty_$wip/accost) | awk '{print $1,$2+$3}' >$dir/scoring_kaldi/penalty_$wip/cost
      export LC_ALL=en_US.UTF-8 
      python3 local/plot_scatter.py --wake-word $wake_word $dir/scoring_kaldi/penalty_$wip/cost $data/utt2dur $data/text
      export LC_ALL=C
    fi
