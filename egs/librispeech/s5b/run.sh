...
rir_dir=data/rir
keep_frac=0.5
num_train_replicas=8
label_type=rir  # To use room id as label, set this to "room"
...
if [ $stage -le 4 ]; then
  ...
  # utterance has 2 corresponding labels: the room ID and the RIR
  # ID, either of which may be used for training. The RIR ID
  # is a more fine-grained classification. These are stored in the
  # files utt2room and utt2rir.
  ...
  local/prepare_rir_data.sh --stage $stage --rir-dir $rir_dir \
    --keep-frac $keep_frac --label-type $label_type \
    --num-train-replicas $num_train_replicas 
fi
...
if [ $stage -le 6 ]; then
  ...
  # have multiple copies of the same utterance with difference RIR
  # augmentations.
...
if [ $stage -le 7 ]; then
  ...
  for datadir in train_clean_100 train_rir_100; do
    local/nnet3/xvector/prepare_feats_for_egs.sh --nj 40 --cmd "$train_cmd" \
      data/$datadir data/${datadir}_no_sil exp/${datadir}_no_sil
    utils/fix_data_dir.sh data/${datadir}_no_sil
  done
fi
...
# At this stage you can choose if you want to train the r-vectors
# using the Kaldi chain model or using a Pytorch based 
# implementation.
...
# Use the --clean-data argument to pass MFCCs for the
# clean (no RIR augmentation) utterance corresponding to each
# augmented utterance. This is used as an additional 
# objective in a multi-task setting.
# Also, note that we reuse the examples already generated
# in the Kaldi xvector script.
if [ $stage -le 13 ]; then
  feat_dim=40
  num_classes=$(< "data/train_rir_100_no_sil/spk2utt" wc -l)
  steps/train_rvec.sh --stage $stage --train-stage -1 \
    --data data/train_rir_100_no_sil --nnet-dir $nnet_dir \
    --clean-data data/train_clean_100_no_sil \
    --egs-dir exp/xvector_rir_nnet_1a/egs \
    --feat-dim $feat_dim --num-classes $num_classes
fi
