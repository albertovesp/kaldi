...
# Apache 2.0
#
# This script dumps training examples (egs) for parameter prediction based
# rvector training. These egs consist of a data chunk and RIR parameters.
...
cp $data/utt2params $dir/temp/utt2params
...
if [ $stage -le 0 ]; then
  ...
  utils/filter_scp.pl $temp/utt2num_frames.train $temp/utt2params > $temp/utt2params.train
  utils/filter_scp.pl $temp/utt2num_frames.valid $temp/utt2params > $temp/utt2params.valid
  utils/filter_scp.pl $temp/utt2num_frames.train_subset $temp/utt2params > $temp/utt2params.train_subset
fi
...
if [ $stage -le 2 ]; then
  ...
  $cmd $dir/log/allocate_examples_train.log \
    local/nnet3/xvector/allocate_egs.py \
      ...
      --utt2int-filename=$dir/temp/utt2int.train \
      --utt2params-filename=$dir/temp/utt2params.train --egs-dir=$dir  || exit 1
  ...
  $cmd $dir/log/allocate_examples_train_subset.log \
    local/nnet3/xvector/allocate_egs.py \
      ...
      --utt2int-filename=$dir/temp/utt2int.train_subset \
      --utt2params-filename=$dir/temp/utt2params.train_subset --egs-dir=$dir  || exit 1
  ...
  $cmd $dir/log/allocate_examples_valid.log \
    local/nnet3/xvector/allocate_egs.py \
      ...
      --utt2int-filename=$dir/temp/utt2int.valid \
      --utt2params-filename=$dir/temp/utt2params.valid --egs-dir=$dir  || exit 1
fi
...
if [ $stage -le 3 ]; then
  ...
  for g in $(seq $nj); do
    ...
    $cmd $dir/log/train_create_examples.$g.log \
      nnet3-rvector-get-egs --compress=$compress --num-pdfs=$num_pdfs $temp/ranges.$g \
  ...
  $cmd $dir/log/train_subset_create_examples.1.log \
    nnet3-rvector-get-egs --compress=$compress --num-pdfs=$num_pdfs $temp/train_subset_ranges.1 \
  ...
  $cmd $dir/log/valid_create_examples.1.log \
    nnet3-rvector-get-egs --compress=$compress --num-pdfs=$num_pdfs $temp/valid_ranges.1 \
