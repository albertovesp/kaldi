  ...
  if [ ! -z "$ali_or_decode_dir" ]; then
    $cmd --num-threads $num_threads JOB=1:$nj $dir/log/extract_ivectors.JOB.log \
      ...
      ivector-extract --num-threads=$num_threads --acoustic-weight=$posterior_scale --compute-objf-change=true \
        --max-count=$max_count \
      $srcdir/final.ie "$feats" ark,s,cs:- ark,t:$dir/ivectors.JOB.ark || exit 1;
  else
    $cmd --num-threads $num_threads JOB=1:$nj $dir/log/extract_ivectors.JOB.log \
      gmm-global-get-post --n=$num_gselect --min-post=$min_post $srcdir/final.dubm "$gmm_feats" ark:- \| \
      ivector-extract --num-threads=$num_threads --acoustic-weight=$posterior_scale --compute-objf-change=true \
...
if [ $stage -le 3 ]; then
  # Average the utterance-level ivectors to get speaker-level xvectors.
  echo "$0: computing mean of ivectors for each speaker"
  $cmd JOB=1:$nj $dir/log/speaker_mean.JOB.log \
    ivector-mean ark:$this_sdata/JOB/spk2utt scp:$dir/ivectors.JOB.scp \
    ark,t:$dir/ivectors_spk.JOB.ark ark,t:$dir/num_utts.JOB.ark || exit 1;
    #ark,t:$dir/ivectors_spk.JOB.ark,$dir/ivectors_spk.JOB.scp ark,t:$dir/num_utts.JOB.ark || exit 1;
fi
