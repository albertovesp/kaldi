...
if [ $stage -le 2 ]; then
  ...
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $mfccdir/storage ]; then
    utils/create_split_dir.pl /export/a0{5,6,7,8}/$USER/kaldi-data/mfcc/wsj-$(date +'%m_%d_%H_%M')/s5/$mfccdir/storage $mfccdir/storage
  fi
...
if [ $stage -le 5 ]; then
  ...
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $ivectordir/storage ]; then
    utils/create_split_dir.pl /export/a0{5,6,7,8}/$USER/kaldi-data/ivectors/wsj-$(date +'%m_%d_%H_%M')/s5/$ivectordir/storage $ivectordir/storage
  fi
  ...
  steps/online/nnet2/extract_xvectors.sh --cmd "$train_cmd" --nj $nj \
  ...
  for data in ${test_sets}; do
    ...
    steps/online/nnet2/extract_xvectors.sh --cmd "$train_cmd" --nj "${nspk}" \
