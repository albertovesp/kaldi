...
def main():
    ...
    parser.add_argument('dur_file', type=str, help='duration file')
    ...
    with open(args.dur_file) as f:
        for line in f:
            assert line.strip().split()[0] not in dur
            dur[line.strip().split()[0]] = float(line.strip().split()[1])
    ...
    #score = [-cost[entry[0]] / dur[entry[0]] for entry in text]
