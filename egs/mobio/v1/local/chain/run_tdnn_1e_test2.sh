...
lang=data/lang_chain_test2
lang_decode=data/lang_chain_decode_test2
tree_dir=exp/chain/tree_test2  # it's actually just a trivial tree (no tree building)
...
if [ $stage -le 10 ]; then
  ...
  if [ -d $lang ]; then
    ...
    id_word=`cat $lang/phones.txt | grep "hixiaowen" | awk '{print $2}'`
    id_freetext=`cat $lang/phones.txt | grep "freetext" | awk '{print $2}'`
    # Use our special topology... note that later on may have to tune this
    # topology.
    local/gen_topo2.pl 20 4 1 $id_word $id_freetext $silphonelist >$lang/topo
  fi
fi
...
if [ $stage -le 16 ]; then
  ...
  local/gen_topo2.pl 20 4 1 $id $freetext_id $sil_id >$lang_decode/topo
  mkdir -p $lang_decode/lm
  cat <<EOF > $lang_decode/lm/fst.txt
0 1 $sil_id $sil_id
1 4 $freetext_id $freetext_id 0.3
4 0 $sil_id $sil_id
1 2 $id $id 2.0
2 0 $sil_id $sil_id
0
