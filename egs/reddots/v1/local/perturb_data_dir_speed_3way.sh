...
utils/data/perturb_data_dir_speed.sh 0.5 ${srcdir} ${destdir}_speed0.5 || exit 1
utils/data/perturb_data_dir_speed.sh 1.5 ${srcdir} ${destdir}_speed1.5 || exit 1
...
if $always_include_prefix; then
  ...
  utils/data/combine_data.sh $destdir ${destdir}_speed1.0 ${destdir}_speed0.5 ${destdir}_speed1.5 || exit 1

  rm -r ${destdir}_speed0.5 ${destdir}_speed1.5 ${destdir}_speed1.0
else
  utils/data/combine_data.sh $destdir ${srcdir} ${destdir}_speed0.5 ${destdir}_speed1.5 || exit 1
  rm -r ${destdir}_speed0.5 ${destdir}_speed1.5
fi
