...
# 1j is as 1g but uses a combination of x-vectors and
...
affix=1j   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
if [ $stage -le 1 ]; then
  # Concat i-vectors and x-vectors for train set
  ivector_dir=exp/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
  xvector_dir=exp/nnet3${nnet3_affix}/xvectors_${train_set}_sp_hires_${xvector_dim}
  out_dir=exp/nnet3${nnet3_affix}/ixvectors_${train_set}_sp_hires
  mkdir -p ${out_dir}

  $train_cmd $out_dir/log/paste_feats.log \
    paste-feats scp:${ivector_dir}/ivector_online.scp scp:${xvector_dir}/ivector_online.scp \
      ark,scp:${out_dir}/ivector_online.ark,${out_dir}/ivector_online.scp
  
  # Also concat vectors for test sets
  for datadir in $test_sets; do
    ivector_dir=exp/nnet3${nnet3_affix}/ivectors_${datadir}_hires
    xvector_dir=exp/nnet3${nnet3_affix}/xvectors_${datadir}_hires_${xvector_dim}
    out_dir=exp/nnet3${nnet3_affix}/ixvectors_${datadir}_hires
    mkdir -p ${out_dir}
...
train_ivector_dir=exp/nnet3${nnet3_affix}/ixvectors_${train_set}_sp_hires
    ...
    (
      ...
      for lmtype in tgpr bd_tgpr; do
        steps/nnet3/decode.sh \
          ...
          --online-ivector-dir exp/nnet3${nnet3_affix}/ixvectors_${data}_hires \
