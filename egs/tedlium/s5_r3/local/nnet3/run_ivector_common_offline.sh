...
if [ $stage -le 3 ]; then
  ...
  steps/nnet/ivector/train_diag_ubm2.sh --cmd "$train_cmd" --nj $nj \
    --num-frames 700000 \
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet/ivector/train_ivector_extractor2.sh --cmd "$train_cmd" \
    --nj $nj --ivector-dim 600 \
...
if [ $stage -le 5 ]; then
  ...
  # We now extract iVectors on the speed-perturbed training data.
