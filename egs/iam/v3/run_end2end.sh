...
if [ $stage -le 3 ]; then
  for set in test train val; do
    ...
    utils/lang/bpe/apply_unigram_sr.py -i $tempfile -m 'data/local/unigram_sr' \
      -o data/$set/unigram_sr_text
    rm $tempfile
    mv data/$set/text data/$set/text.old
    paste -d' ' data/$set/ids data/$set/unigram_sr_text > data/$set/text
  done
fi
...
if [ $stage -le 5 ]; then
  ...
  # This recipe uses subword segmentation, the silences are part of the words' pronunciations.
