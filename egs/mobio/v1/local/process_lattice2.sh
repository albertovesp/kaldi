...
if [ $stage -le 4 ]; then
  ...
  paste -d' ' <(cut -f1,2 -d' ' $dir/scoring/cost.txt) <(cat $data/utt2dur) | awk '{if ($1==$3) {a=-$2/$4;print $1,a}}' >$dir/scoring/score.txt
  #cat $dir/scoring/cost.txt | awk '{a=-$2;print $1,a}' > $dir/scoring/score.txt
