...
# 1k is as 1i but uses x-vectors for speaker adaptation.
...
xvec_extractor=exp/xvector_nnet_1a
...
tdnn_affix=1k3  #affix for TDNN directory, e.g. "a" or "b", in case we change the configuration.
...
train_ivector_dir=exp/$mic/nnet3${nnet3_affix}/xvectors_${train_set}_sp_hires
...
if [ $stage -le 15 ]; then
  ...
  no-op-component name=noop1 input=Append(lda,Scale(0.5,ReplaceIndex(ivector,t,0)))
  # the first splicing is moved before the lda layer, so no splicing here
  relu-batchnorm-layer name=tdnn1 dim=450 $opts input=noop1
