...
lang_decode=data/lang_chain_decode_test
tree_dir=exp/chain/tree_test  # it's actually just a trivial tree (no tree building)
...
0 1 $id_sil $id_sil
1 2 $id_word $id_word 1.9
2 3 $id_sil $id_sil
1 4 $id_freetext $id_freetext 0.7
4 3 $id_sil $id_sil
3
EOF
#3 1.9
...
0
EOF
#0 4 $freetext_id $freetext_id 10.0
#0 2 $id $id 10.0
#2
#4
#1.91 -3.0
...
if $test_online_decoding && [ $stage -le 20 ]; then
  steps/online/nnet3/prepare_online_decoding.sh \
    --mfcc-config conf/mfcc_hires.conf $lang_decode $dir ${dir}_online
  ...
  for data in $test_sets; do
    (
      ...
      steps/online/nnet3/decode.sh \
        ...
        $tree_dir/graph data/${data} ${dir}_online/decode_${data} || exit
...
if $test_online_decoding && [ $stage -le 21 ]; then
  for data in $test_sets; do
    python3 local/plot_roc.py ${dir}_online/decode_${data}/scoring_kaldi/all_results
    nspk=$(wc -l <data/${data}_hires/spk2utt)
    local/process_lattice.sh --nj $nspk --wake-word $wake_word ${dir}_online/decode_${data} data/${data}_hires $lang || exit 1
  done
  echo "Done online. Date: $(date). Results:"
  local/chain/compare_wer.sh ${dir}_online
fi
